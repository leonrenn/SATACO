...
from utils.loading import read_corr_matrix
...
def main() -> int:
    ...
    # checks if the parser received the correlation matrix
    ...
    if parser_dict["corr_matrix"] is None:
        ...
        try:
            correlation_matrix, SR_names = read_corr_matrix(
                parser_dict["corr_matrix"])
            non_zero_column_names = SR_names
        except CorrelationMatrixFormatError or FileNotFoundError:
            print(f"File {parser_dict['corr_matrix']} is either not "
                  "found or in the wrong format.")
            return 10
        weights_SR = None
        sorted_dict_SR_weigths = dict(
            zip(non_zero_column_names,
                np.linspace(start=1.0,
                            stop=1.0,
                            num=len(non_zero_column_names))))
